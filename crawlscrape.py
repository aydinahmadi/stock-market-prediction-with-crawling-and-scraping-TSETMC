...
def get_excel(stock_name):
    ...
    query3 = " SELECT * FROM Stocks.stocks WHERE name = %s "
    try:
        conn = msql.connect(host='localhost', database='Stocks', user='root', password='13772010')
        if conn.is_connected():
            cursor = conn.cursor(buffered=True)
            cursor.execute("select database();")
            record = cursor.fetchone()
            cursor.execute(query3, (stock_name,))
            rows = cursor.fetchall()
            data = pd.DataFrame(rows,
                                columns=['name', 'date', 'number', 'volume', 'first', 'high', 'low', 'value', 'last',
                                         'close'])
            conn.commit()
            filename_needed = 'C:/Users/aidin/Downloads/'
